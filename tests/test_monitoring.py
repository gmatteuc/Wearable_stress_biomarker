...
from src.monitoring.drift_report import compute_drift  # noqa: E402
...
def test_monitoring():
    ...
    df_ref = df[df["subject_id"].isin(ref_subjects)].sample(
        100, replace=True
    )  # Sample for speed
    ...
    logger.info("-" * 30)
    logger.info(
        f"Drift Report Summary: {drift_count}/{total_features} features drifted."
    )
    ...
    if "EDA_mean" in report:
        ...
        logger.info(
            f"EDA Mean Drift: {eda_drift['drift_detected']} (p={eda_drift['p_value']:.4f})"
        )
    ...
    if drift_count > 0:
        print(
            "SUCCESS: Drift Detection logic works (Physiological differences detected)."
        )
    else:
        print(
            "WARNING: No drift detected. This is unusual for different subjects but possible if normalized."
        )
