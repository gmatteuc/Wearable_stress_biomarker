...
class SignalQualityIndex:
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.flatline_thresholds = {
            **defaults_flatline,
            **sqi_cfg.get("flatline_thresholds", {}),
        }
    ...
    def compute_sqi(self, window_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        for _, sig in window_data.items():
        ...
        if "EDA" in window_data and sqi_scores["completeness"] == 1.0:
            ...
            if eda_range < self.flatline_thresholds["EDA"]:
                ...
                np.mean(eda) < self.valid_ranges["EDA"][0]
                or np.mean(eda) > self.valid_ranges["EDA"][1]
            ):
                sqi_scores["eda_sqi"] = 0.0
            else:
        ...
        if "TEMP" in window_data and sqi_scores["completeness"] == 1.0:
            ...
            # Check 2: Plausible Biological Range (20-45C)
                ...
                np.mean(temp) < self.valid_ranges["TEMP"][0]
                or np.mean(temp) > self.valid_ranges["TEMP"][1]
            ):
                sqi_scores["temp_sqi"] = 0.0
            else:
        ...
        if "ACC_x" in window_data and sqi_scores["completeness"] == 1.0:
            ...
            sqi_scores["motion_score"] = max(
                0.0, 1.0 - (acc_std / self.acc_noise_threshold)
            )
    ...
    def is_acceptable(
        self, window_data: Dict[str, Any], threshold: float = None
    ) -> bool:
