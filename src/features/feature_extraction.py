...
class FeatureExtractor:
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # Load EDA threshold
        self.eda_threshold = (
            self.config.get("features", {}).get("eda", {}).get("scr_threshold", 0.015)
        )
    ...
    def process_windows(self, windows_data) -> "pd.DataFrame":
        ...
        for w in windows_list:
            ...
            meta = {
                k: v
                for k, v in w.items()
                if k in ["subject_id", "label", "session", "start_idx"]
            }
    ...
    def get_eda_components(
        ...
        peaks, _ = signal.find_peaks(
            phasic, prominence=self.eda_threshold, distance=self.fs
        )
    ...
    def _process_acc(self, window: Dict[str, Any]) -> Dict[str, float]:
        ...
        feats = {
            ...
            "acc_energy": np.sum(np.square(mag - np.mean(mag)))
            / len(mag),  # Dynamic Energy
        }
        return feats
    ...
    def get_ecg_peaks(self, sig: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
        ...
        """
        b, a = signal.butter(
            3, [1.0 / (0.5 * self.fs), 15.0 / (0.5 * self.fs)], btype="band"
        )
        ...
        peaks, _ = signal.find_peaks(
            clean_ecg, height=peak_threshold, distance=int(0.35 * self.fs)
        )
    ...
    def _process_ecg(self, sig: np.ndarray) -> Dict[str, float]:
        ...
        feats = {"ecg_hr_bpm": hr_bpm, "ecg_rmssd": rmssd, "ecg_sdnn": sdnn}
    ...
    def _process_resp(self, sig: np.ndarray) -> Dict[str, float]:
        ...
        if np.any(valid_mask):
            ...
            peak_indices, properties = signal.find_peaks(
                Pxx_valid, height=max_power * 0.20
            )
            ...
            if len(peak_indices) > 0:
                ...
                for i, f_p in enumerate(peak_freqs):
                    ...
                    if abs(f_p - potential_fundamental) < max(
                        0.04, 0.1 * potential_fundamental
                    ):
                ...
                for i, f_p in enumerate(peak_freqs):
                    if (
                        abs(f_p - potential_fundamental_3) < 0.03
                        and peak_heights[i] > 0.3 * peak_heights[best_idx]
                    ):
                        dominant_f = f_p
                        break
    ...
    def _process_bvp(self, sig: np.ndarray) -> Dict[str, float]:
        ...
        b, a = signal.butter(
            3, [0.5 / (0.5 * self.fs), 8.0 / (0.5 * self.fs)], btype="band"
        )
        clean_bvp = signal.filtfilt(b, a, sig)

        # Stat features
        feats = {"bvp_mean": np.mean(sig), "bvp_std": np.std(sig)}
        ...
        peaks, _ = signal.find_peaks(
            sq_bvp, height=peak_threshold, distance=int(0.35 * self.fs)
        )
