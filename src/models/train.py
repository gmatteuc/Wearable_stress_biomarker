...
class Trainer:
    def __init__(self, model_type: str, split_type: str):
        ...
        self.run_dir = (
            PROJECT_ROOT
            / "reports"
            / f"{model_type}_{split_type}_{sensor_loc}_{timestamp}"
        )
        ...
        if self.model_type == "deep":
            ...
            if not file_path.exists():
                raise FileNotFoundError(
                    "features.parquet not found. Run 'make features' first."
                )
    ...
    def get_split(self, groups):
        if self.split_type == "loso":
            ...
            train_idx, test_idx = next(
                gkf.split(X=np.zeros(len(groups)), groups=groups)
            )
            ...
            train_idx, test_idx = train_test_split(
                indices, test_size=self.config["training"]["test_size"], random_state=42
            )
            return train_idx, test_idx
    ...
    def train_classical(self):
        ...
        if unique_labels == [1, 2]:
            label_map = {1: 0, 2: 1}
            labels = [
                self.config["data"]["labels"][1],
                self.config["data"]["labels"][2],
            ]
        else:
            label_map = {lbl: i for i, lbl in enumerate(unique_labels)}
            labels = [self.config["data"]["labels"][i] for i in unique_labels]
        y_mapped = np.array([label_map[lbl] for lbl in y])
        ...
        if self.split_type == "loso":
            ...
            X_train, X_test = X[train_idx], X[test_idx]
            y_train, y_test = y_mapped[train_idx], y_mapped[test_idx]
            _, groups_test = groups[train_idx], groups[test_idx]
        ...
        try:
            ...
            fig_diag = plot_model_diagnostics(
                results_df, save_folder=None, title_prefix=prefix
            )
            fig_diag.savefig(
                self.run_dir / "model_diagnostics.png", dpi=300, bbox_inches="tight"
            )
            plt.close(fig_diag)
            ...
            fig_unc = plot_confidence_abstention_panel(
                results_df,
            ...
            fig_unc.savefig(
                self.run_dir / "confidence_analysis.png", dpi=300, bbox_inches="tight"
            )
            plt.close(fig_unc)
    ...
    def train_deep(self):
        ...
        if unique_labels == [1, 2]:
            ...
            label_map = {lbl: i for i, lbl in enumerate(unique_labels)}
            display_labels = [
                self.config["data"]["labels"][lbl] for lbl in unique_labels
            ]
        y_mapped = np.array([label_map[lbl] for lbl in y])
        ...
        if self.split_type == "loso":
            ...
            for i, (train_idx, test_idx) in enumerate(gkf.split(X, y_mapped, groups)):
                ...
                )  # Need DS for loader if batching test
            ...
            fig_fc.savefig(
                self.run_dir / "learning_curves.png", bbox_inches="tight", dpi=300
            )
            ...
            stress_col = [
                c for c in plot_df.columns if "stress" in c.lower() and "prob" in c
            ]
            if stress_col:
            ...
            if "prob" in plot_df.columns:
                ...
                fig_diag.savefig(
                    self.run_dir / "model_diagnostics.png", bbox_inches="tight", dpi=300
                )

                # 3. Confidence & Abstention
                fig_conf = plot_confidence_abstention_panel(
                    plot_df,
                    confidence_threshold=0.7,
                    title_prefix="Deep LOSO ",
                    save_folder=None,
                )
                fig_conf.savefig(
                    self.run_dir / "confidence_abstention.png",
                    bbox_inches="tight",
                    dpi=300,
                )
            else:
                logger.warning(
                    "Could not identify 'prob' column for Stress. Skipping diagnostic plots."
                )
            ...
            final_model = ResNet1D(X.shape[1], len(unique_labels), X.shape[2]).to(
                device
            )
            ...
            train_dataset = TensorDataset(
                torch.tensor(X_train, dtype=torch.float32),
                torch.tensor(y_mapped[train_idx], dtype=torch.long),
            )
            test_dataset = TensorDataset(
                torch.tensor(X_test, dtype=torch.float32),
                torch.tensor(y_mapped[test_idx], dtype=torch.long),
            )
            ...
            evaluate_model(
                y_trues, y_probs, display_labels, self.run_dir, subject_ids=groups_test
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, choices=["logistic", "deep"], required=True
    )
