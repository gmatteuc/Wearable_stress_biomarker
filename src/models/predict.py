...
class StressPredictor:
    def __init__(self, run_dir: Union[str, Path]):
        ...
        self.class_names = ['Baseline', 'Stress'] # Config might have more, but binary is this.

        # 2. Determine Model Type & Load Model
        self.model = None
        self.model_type = None
        self.device = torch.device('cpu') # Inference usually on CPU unless batch is huge
        ...
        if (self.run_dir / "model.pt").exists():
            ...
            raise ValueError("No valid model file (model.pt or model.joblib) found in run directory.")
    ...
    def predict(self, data: Union[pd.DataFrame, Dict[str, np.ndarray], str]) -> Dict[str, Any]:
        """
    ...
    def _predict_deep(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if missing:
             ...
             logger.warning(f"Missing columns for Deep Model: {missing}. Filling with 0.")
             for c in missing:
                 df[c] = 0.0
        ...
        # Abstention Logic
        conf_threshold = self.config['training'].get('models', {}).get('deep', {}).get('confidence_threshold', 0.7)
    ...
    def _predict_classical(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        abstained = False # Default strict
