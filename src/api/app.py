...
def load_model():
    global predictor
    try:
        # Find latest Deep Learning Artifact
        reports_dir = PROJECT_ROOT / "reports"
        deep_runs = sorted(list(reports_dir.glob("deep_loso_CHEST_*")))
        
        if not deep_runs:
            logger.error("No trained models found in reports/!")
            return
            
        latest_run = deep_runs[-1]
        logger.info(f"Loading Model Artifact: {latest_run.name}")
        
        predictor = StressPredictor(latest_run)
        logger.info(f"StressPredictor initialized successfully ({predictor.model_type}).")
