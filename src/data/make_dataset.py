...
class WESADPreprocessor:
    ...
    def process_subject(self, subject_id: str) -> List[Dict]:
        ...
        if location == "chest":
            ...
            def process_chest(key, dims):
                ...
                if dims == 1:
                    ...
                    return np.stack(
                        [self.resample_signal(raw[:, i], 700) for i in range(dims)],
                        axis=1,
                    )
            ...
            def process_wrist(key, dims, original_fs):
                ...
                if dims == 1:
                    ...
                    return np.stack(
                        [
                            self.resample_signal(raw[:, i], original_fs)
                            for i in range(dims)
                        ],
                        axis=1,
                    )
        ...
        # Use +1 to include the final window if it fits exactly
        for start in range(0, num_samples - window_len + 1, int(step)):
